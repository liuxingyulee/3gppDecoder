...
if empty? nats [
    warn-button-popup "错误" "配置文件（3gppDecoder.cfg）中的协议为空，请检查配置文件"
]

selected-proto: ""

proc-hex-stream: function [
    src-str [string!]
][
    ...
    parse src-str [
        some[some[whitespace] (hex-ind: 0) | [pos: hex-digits] (
            either hex-ind == 0 [
                append dst-str " 0"
...
main-window: layout [
    ...
    decode-btn: button "解码" [
        clear output-area/text
        if all [selected-proto <> "" input-area/text <> ""][
            decode-by-tshark selected-proto input-area/text
        ]
    ]
    npp-btn: button "用notepad++打开" [
        clear output-area/text
        if all [selected-proto <> "" input-area/text <> ""][
            decode-by-tshark selected-proto input-area/text
    ...
    ws-btn: button "用wireshark打开" [
        if all [selected-proto <> "" input-area/text <> ""][
            decode-by-wireshark selected-proto input-area/text
        ]
    ]
    clr-btn: button "清空" [
        input-area/text: ""
